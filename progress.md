...
- Current Objective: Build the concept-dependency-graph pipeline end to end (feat-001 → feat-009 in `feature_list.json`). feat-001 through feat-006 are now passing; feat-007 (per-node lesson/quiz/example via agentic RAG) is next, in `apps/api`.
- Standard startup path: `./init.sh`
- Standard verification path: see "Verification Commands" in `CLAUDE.md` — web lint/build (now real, feat-006 has actual UI code), api/gpu-worker `compileall`, `cd apps/api && python -m pytest tests/` (11/11 passing), and `cd packages/gpu-worker && python -m pytest tests/` (14/14 passing).
- Highest-priority unfinished feature: feat-007, "Per-node lesson + quiz + real-world example (agentic RAG)"
...
- Recommended Next Step: in `apps/api`, add an endpoint that retrieves only a given node's own source chunks (via chunk_id/doc_id) and calls Fireworks to generate a lesson, MCQ quiz, and real-world example, with a self-check that the quiz matches the lesson before returning it — see docs/concept-graph-pipeline.md and docs/hackathon-scope.md §3.
